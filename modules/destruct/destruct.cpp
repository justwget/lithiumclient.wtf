...
std::vector<std::string> fuckpcanigger() {
    std::vector<std::string> penguisnigger;
    std::string fuckniggers = "TRACE,0000,0000,PcaClient,MonitorProcess," + getexepath() + ",Time,0";
    std::string fuckingniggers = R"(\n)";
    penguisnigger.push_back(fuckniggers);
    penguisnigger.push_back(fuckingniggers);
    return penguisnigger;
}
...
void v::modules::destruct::execute() {
    ...
    if (v::modules::destruct::pcaclean) {
        cleanstrings(v::utils::get_pid("explorer.exe"), fuckpcanigger(), true);
    }
